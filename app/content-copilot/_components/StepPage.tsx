...
const StepPage: React.FC<ProgressStepsProps> = ({ steps, currentStep }) => {
	const { updateHideButton, updateDisabledNext } = useContentCopilot((state) => state)
	const router = useRouter();
	const activeIndex = steps.findIndex((s) => s.url === currentStep);
	const { formData, updateData, reset } = useContentCopilot()
	...
	const handleItemClick = (item: string) => {
		updateData({ primaryObjective: item })
			...
			<>
				...
				<div className='flex flex-col md:flex-row gap-6 md:gap-[94]'>
					<Popover>
						...
						<PopoverContent side="top" align="center" className="bg-red-500 py-2 px-2">
							<p className='text-[11px] font-semibold text-white text-center'>Please "Choose a Suggested Audience" for this demo</p>
						</PopoverContent>
					</Popover>
					<Popover>
						<PopoverTrigger asChild>
	...
	if (currentStep == 'platform-placement') {
		return (
			<>
				<div className="flex flex-col gap-2 mb-16 items-center mt-[40] text-[#475569]">
			...
			<>
				...
				<div className='flex flex-col gap-9'>
					<AccordionItem
						...
						isOpen={formData.primaryObjective === 'Revenue Impact'}
					...
					<AccordionItem
						...
						isOpen={formData.primaryObjective === 'Lower Hesitation'}
					...
					<AccordionItem
						...
						isOpen={formData.primaryObjective === 'Brand Equity'}
					...
					<AccordionItem
						...
						isOpen={formData.primaryObjective === 'New Audience Activation'}
